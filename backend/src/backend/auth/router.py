...
from backend.db.models import School, Teacher
...
def me(
    ...
    out = TeacherOut.model_validate(current_teacher)
    if current_teacher.school_id:
        school = db.get(School, current_teacher.school_id)
        if school:
            out.school_name = school.name
            out.school_udise_code = school.udise_code
    return out
